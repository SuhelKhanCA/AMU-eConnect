...
1. **Clone the Repository**:
   ```bash
   Step 1: git clone https://github.com/SuhelKhanCA/AMU-Econnect.git
   Step 2: cd AMU-Econnect/eConnect
   ```

2. **Set Up a Virtual Environment**:
    ```bash    
      Step1: python -m venv venv
      Step 2: source venv/bin/activate   # On Windows, use `venv\Scripts\activate`
    ```
...
│   ├── forms.py          # Forms for user input and validation
│   └── __init__.py       # Flask app initialization
├── migrations/           # Database migrations (if used)
├── requirements.txt      # Python dependencies
├── README.md             # Project documentation
└── econect.py               # Application entry point
